...
export const FourWorlds: React.FC = () => {
  const cards = [
    ...
    },
    {
      ...
      badge: 'Location',
      badgeBg: 'bg-[#1A1615] text-[#FAF8F3]',
      accentColor: 'text-[#1A1615]',
      borderHover: 'hover:border-[#1A1615]/50 hover:shadow-[#1A1615]/10',
      btnBg: 'bg-[#1A1615]/10 text-[#1A1615] group-hover:bg-[#1A1615] group-hover:text-white',
    },
    {
      id: 'luxe',
    ...
    <section id="universes" className="py-20 sm:py-28 bg-[#FAF8F3]">
      ...
      <div className="max-w-[1180px] mx-auto px-4 sm:px-8 mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        >
          ...
          <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl text-[#1A1615] font-semibold max-w-[16ch] leading-tight">
        ...
        <motion.p
          ...
          className="text-sm sm:text-base text-[#443E3B] max-w-[36ch] leading-relaxed"
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {cards.map((card, idx) => (
            ...
            >
              <Link
                href={card.href}
                className={`group relative bg-[#FAF8F3] border border-[rgba(26,22,21,0.14)] rounded-xs overflow-hidden flex flex-col justify-between h-full hover:shadow-2xl transition-all duration-300 ${card.borderHover}`}
              >
                <div>
                  {/* Photo Header */}
                  ...
                  {/* Body Content */}
                  <div className="p-6">
                    <h3 className="font-serif text-xl font-semibold text-[#1A1615] mb-1.5 group-hover:text-[#B15A3C] transition-colors">
                      {card.title}
                    </h3>
                    <p className={`text-xs font-semibold mb-3 ${card.accentColor}`}>
                      {card.subtitle}
                    </p>
                    <p className="text-xs sm:text-sm text-[#443E3B] leading-relaxed">
                      {card.desc}
                    </p>
                  </div>
                </div>
                ...
                {/* Footer Link */}
                <div className="px-6 pb-6 pt-2 flex items-center justify-between border-t border-[rgba(26,22,21,0.08)] mt-4">
